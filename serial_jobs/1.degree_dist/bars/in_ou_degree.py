...
def deg_dist_combined (in_d_count, ou_d_count, ax, xlims, display_x_label, title="Degree distribution", colors=['forestgreen','#2F67B1'], tick_interval=2.0):
    ...
    while max_x%2 !=0:
        max_x+=1
    xticks = [tick for tick in range(0, max_x, int(tick_interval))]
    
    selected_ticks = [t for t in xticks if t%2==0]
    if len(selected_ticks)==0:
        #print ("ticks > "+str(xticks))
        selected_ticks = selected_ticks = [t for t in xticks if t%2!=0]
    #force tick 0
    selected_ticks = list(set([0]+selected_ticks))
    print ("selected_ticks > "+str(selected_ticks))
