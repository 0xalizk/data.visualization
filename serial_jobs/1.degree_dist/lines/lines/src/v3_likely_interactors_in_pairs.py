...
def getCommandLineArgs():
    ...
    try:
        ...
        print ("Usage: python3 *.py [/absolute/path/to/input/file.txt (containing abs paths to edge files)]\nExiting..\n")
        sys.exit() 
####################################################################
def formatter(x, y):
    if x<=0:
...
def save_figure(network_file):
    ...
    plt.savefig(plot_dir+'v3_'+file_name+".png", dpi=100,bbox_inches="tight") # http://matplotlib.org/api/figure_api.html#matplotlib.figure.Figure.savefig
    print ("plotted: "+plot_dir+'v3_'+file_name+".png")
####################################################################
def logify(SIZES):
    mins = min(SIZES)
    offset=0
...
def scatter_v1(interaction_likelihoods, colors, sizes, ax, title, fig, xlabel, ylabel, log=False):    
    ...
    return ax
####################################################################
def scatter(interaction_likelihoods, colors, sizes, ax, title, fig, xlabel, ylabel, log=False):    
    sc = None
    Xs, Ys, Ss, Cs, lolim, hilim = [], [], [], [], 0, 0
    for (source_deg,target_deg) in interaction_likelihoods.keys():
        Xs.append (source_deg)
    ...
    #sc=ax.scatter (Xs, Ys, alpha=.7, marker='o', edgecolors='',s=[100*s for s in Ss],  c=Cs, cmap=plt.cm.get_cmap('plasma')) 
    sc=ax.scatter (Xs, Ys, alpha=.9, marker='o', edgecolors='', c=Cs, cmap=plt.cm.get_cmap('plasma')) 
    if log==True:
        ax.set_yscale('log', basex=2, basey=2, subsx=[0,2,4,8,16], subsy=[0,2,4,8,16])
        ax.set_xscale('log', basex=2, basey=2, subsx=[0,2,4,8,16], subsy=[0,2,4,8,16])
    else:
...
def get_likely_interactors (M):
    ...
    for X in all_degrees:
        tmp  = [key for key in sizes.keys() if key[0]==X]
        print(str(sum([sizes[key] for key in tmp])))
    sys.exit(1)
    '''
    print("="*100+'\n'+'='*100)
    ...
    '''
    print(str( sum(colors.values()))) 
    print(str( sum(sizes.values()))) 
    print(str(sum(M.out_degree().values())))
    #sys.exit(1)
...
def getCOORDs (fig_inch_dims, cols, rows, w2h_ratio=0.5):
    ...
    return  SORTED
##################################################################
def v1(network_files, clean_titles, output_file_name):
    update_rcParams()
...
if __name__ == "__main__":
    ...
    for f,title, log in zip(network_files, clean_titles, [True]*len(network_files)):
        ...
        xlabel, ylabel = "Degree of source node", "Degree of target node"
        ax = scatter(likely_interactors, colors, sizes, ax, "", fig, xlabel, ylabel, log) 
        pos+=cols
    print('\nsaving..')
    save_figure(output_file_name)
